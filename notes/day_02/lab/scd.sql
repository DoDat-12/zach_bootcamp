...
--purpose: track change is_active in amount of time
...
FROM with_streaks
GROUP BY player_name, is_active, scoring_class, streak_identidier
ORDER BY player_name, streak_identidier;
...
WITH last_season_scd AS (SELECT * FROM players_scd WHERE current_season = 2021 AND end_season = 2021),
     historical_scd AS (SELECT player_name,
                        ...
                        WHERE current_season = 2021
                          AND end_season < 2021),
     this_season_data AS (SELECT * FROM players WHERE current_season = 2022),
     unchange_records AS (SELECT ts.player_name,
                                 ts.scoring_class,
                                 ts.is_active,
                                 ls.start_season,
                                 ts.current_season AS end_season --increase by one because of unchanging
                          FROM this_season_data ts
                                   JOIN last_season_scd ls
                                        ON ts.player_name = ls.player_name --get only unchanging
